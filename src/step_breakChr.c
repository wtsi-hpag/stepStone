...
static int *hit_reads,*hit_index1,*hit_index2;
...
/*  read the alignment files         */
    i=0;
    max_ctg = 0;
    while(fscanf(namef,"%s %d %d %d %d %d %s %s %s %s",rdname,&hit_index1[i],&hit_locus1[i],&hit_index2[i],&hit_locus2[i],&hit_reads[i],temp,temp,temp,temp)!=EOF)
    {
        i++;
    }
    fclose(namef);
...
/* =============================== */
{
     ...
     for(i=0;i<nSeq;i++)
     {
        int idi = hit_index1[i];
        int idj = hit_index2[i];

        if((hit_index1[i] == hit_index2[i])&&(hit_reads[i] >= num_reads))
        {
		hit_list[hit_index1[i]]++;
//    printf("link: %d %d %d %d\n",i,idi,idj,r_matrix[5][17]);
//          r_matrix[idj][idi]++;
        } 
     }
     ...
     for(i=1;i<24;i++)
     {
        printf("Chr%d %d %d\n",i,hit_list[i],hit_list[i]/2);
        fprintf(namef2,"Chr%d %d\n",i,hit_list[i]);
     }
     fclose(namef2);
}

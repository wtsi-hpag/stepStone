...
{
    FILE *namef,*namef2;
    int i=0,j=0,m,n,k,args=0,num_steps,num_break2,nSeq,nRead,num_stones;
    int *ref_offset,*ref_offset2,*stn_masked,*stn_reads,*stn_type,*seq_index,*stn_index;
    int *stn_rcdex,*stn_offset,*seq_index2,*stn_loci1,*stn_loci2,stopflag,stopflag2;
    char line[500],**rdname,tmpc[100];
    ...
    if((ref_offset2= (int *)calloc(nRead,sizeof(int))) == NULL)
    {
      printf("ERROR Memory_Allocate: Align_Process - hit_rcdex\n");
      exit(1);
    }
    ...
    if((stn_masked= (int *)calloc(nRead,sizeof(int))) == NULL)
    {
      printf("ERROR Memory_Allocate: Align_Process - hit_rcdex\n");
      exit(1);
    }
    ...
    if((seq_index2= (int *)calloc(nRead,sizeof(int))) == NULL)
    {
      printf("ERROR Memory_Allocate: Align_Process - hit_rcdex\n");
      exit(1);
    }
...
/*  read the alignment files         */
    i=0;
    num_stones = 0;
    while(fscanf(namef,"%d %d %d %d %d %d %s %d %d %s %d",&stn_index[i],&seq_index[i],&ref_offset[i],&seq_index2[i],&ref_offset2[i],&stn_reads[i],tmpc,&stn_loci1[i],&stn_loci2[i],rdname[i],&stn_rcdex[i])!=EOF)
    {
...
//         printf("www: %d %d %d %d\n",i,ref_offset[i],ref_offset[j],BAR);
       ...
       {
         if(ref_offset2[i]==ref_offset2[j])
         {
           j++;
         }
         else
	  ...
	  int ids_hits = 0;
          for(n=i;n<j;n++)
          {
	     if(stn_reads[n] > num_hits)
	     {
	       num_hits = stn_reads[n];
	       ids_hits = n;
	     }
          }
	  k = ids_hits;
          fprintf(namef,"%d %d %d %d %d %d || %d %d %s %d\n",stn_index[k],seq_index[k],ref_offset[k],seq_index2[k],ref_offset2[k],stn_reads[k],stn_loci1[k],stn_loci2[k],rdname[k],stn_rcdex[k]);
       }
       else
       {
	 k = i;

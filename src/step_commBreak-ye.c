...
static char **S_Name,**Samname,**Comname;
...
//        printf("fgets command error:\n);
    ...
    Samname=cmatrix(0,nseq+1,0,Max_N_NameBase);
    Comname=cmatrix(0,100,0,Max_N_NameBase);

    strncpy(Comname[0],"A-tumor",8);
    strncpy(Comname[1],"B-tumor",8);
    strncpy(Comname[2],"C-tumor",8);
    strncpy(Comname[3],"D-tumor",8);
    strncpy(Comname[4],"E-tumor",8);
    strncpy(Comname[5],"A-conto",8);
    strncpy(Comname[6],"B-conto",8);
    strncpy(Comname[7],"C-conto",8);
    strncpy(Comname[8],"D-conto",8);
    strncpy(Comname[9],"E-conto",8);
...
/*  read the alignment files         */
    i=0;
    max_ctg = 0;
    while(fscanf(namef,"%d %d %d %d %d %s",&hit_index1[i],&hit_locus1[i],&hit_index2[i],&hit_locus2[i],&hit_reads[i],Samname[i])!=EOF)
    {
        i++;
    }
    fclose(namef);
...
/* =============================== */
{
     ...
     int **rmatrix;
     char rdname[60];
     int stopflag,offset,ctg_len,add_len,control_flag,n_scaffs;
     int **imatrix(long nrl,long nrh,long ncl,long nch);


     rmatrix=imatrix(0,100,0,100);
...
/*      search reads with an index > i     */
        ...
        {
          if((hit_locus1[i] == hit_locus1[j])&&(hit_locus2[i] == hit_locus2[j]))
          {
            j++;
          }
          else
	...
	for(n=i;n<j;n++)
	{
	   if(Samname[n][2] == 'c')
	   {
        ...
        if(((j-i)>=2)&&(control_flag == 1))
        {
           int num_hits = j-i;
           int NK = 0;
           int MK = 0;
//        printf("www: %d %d %d %s %s\n",i,hit_locus1[i],hit_locus2[i+1],Samname[i],Samname[i+1]);
           for(n=0;n<num_hits;n++)
           {
		...
		if(n != m)
		{
                  for(k=0;k<10;k++)
		  {
		    if(strcmp(Comname[k],Samname[n+i]) == 0)
		    {
		       NK = k;
		       k = 10;
		    }
                  ...
                  for(k=0;k<10;k++)
		  {
		    if(strcmp(Comname[k],Samname[m+i]) == 0)
		    {
		       MK = k;
		       k = 10;
		    }
		  }
//        printf("kkk: %d %d %d %s %s\n",i,NK,MK,Comname[NK],Comname[MK]);
                  rmatrix[NK][MK]++;
                  rmatrix[MK][NK]++;
		}
              }
     ...
     fclose(namef2);
     printf("            %s %s %s %s %s %s %s %s %s %s\n",Comname[0],Comname[1],Comname[2],Comname[3],Comname[4],Comname[5],Comname[6],Comname[7],Comname[8],Comname[9]);
     for(k=0;k<10;k++)
        printf("%s %8d %8d %8d %8d %8d %8d %8d %8d %8d %8d\n",Comname[k],rmatrix[k][0],rmatrix[k][1],rmatrix[k][2],rmatrix[k][3],rmatrix[k][4],rmatrix[k][5],rmatrix[k][6],rmatrix[k][7],rmatrix[k][8],rmatrix[k][9]);
}

...
int main(int argc, char **argv)
{
    ...
    int *ref_offset,*seq_offset,*chr_index,*seq_loci2,*seq_masks,*seq_index;
    ...
    if((seq_loci2= (int *)calloc(nRead,sizeof(int))) == NULL)
    {
      printf("ERROR Memory_Allocate: Align_Process - hit_rcdex\n");
      exit(1);
    }
...
/*  read the alignment files         */
    i=0;
    num_samples = 0;
    while(fscanf(namef,"%s %d %d %s %d %d %d",tmpc,&seq_index[i],&chr_index[i],rdname[i],&ref_offset[i],&seq_offset[i],&seq_loci2[i])!=EOF)
    {
...
//         printf("www: %d %d %d %d\n",i,ref_offset[i],ref_offset[j],BAR);
	 ...
	 for(k=(i+1);k<j;k++)
         {
	    if(chr_index[k]!=chr_index[i])
         printf("%d %s %d %d %d\n",num_steps,rdname[k],chr_index[i],chr_index[k],j-i);
         }
         num_steps++;
       }
       i=j-1;
...
/*  read the alignment files         */
    ...
    {
      fprintf(namef2,"%s %d %s %s %d %d %d\n",tmpc1,seq_masks[i],tmpc2,tmpc3,offset1,offset2,offset3);
      i++;
    }
    fclose(namef);
    fclose(namef2);

...
int main(int argc, char **argv)
{
    ...
    int *stn_links,*stn_link2,*stn_offset,*stn_steps,*stn_loci1,*stn_loci2,stopflag,stopflag2;
...
/*  read the alignment files         */
    i=0;
    num_stones = 0;
    while(fscanf(namef,"%d %d %d %s %s %s %s %s %s %s %s",&stn_index[i],&ref_offset[i],&ref_offset2[i],tmpc,tmpc,tmpc,rdname[i],tmpc,tmpc,tmpc,tmpc)!=EOF)
    {
...
//         printf("www: %d %d %d %d\n",i,ref_offset[i],ref_offset[j],BAR);
       ...
       if((j-i)>=2)
       {
          for(n=i;n<j;n++)
          {
             ...
             {
                if(ref_offset2[n]==ref_offset2[m])
                {
                  m++;
                }
                else
	     ...
	     {
	       for(k=n;k<m;k++)
	          printf("%s %d %d %d\n",rdname[k],ref_offset[k],ref_offset2[k],num_steps);
               num_steps++;
             }
             n=m-1;
          }

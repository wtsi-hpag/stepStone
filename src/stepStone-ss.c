...
int main(int argc, char **argv)
{
    ...
    char file_genome[2000],file_read2[2000],samname[500],bamname[500],toolname[500],fastname[500],datname[500];
    ...
    if(argc < 2)
    {
         ...
         printf("Usage: %s -nodes 30 -data ccs/ont/ngs -bam align.bam <output_breakpoints.vcf>\n",argv[0]);
         ...
         printf("       -data     ngs     - Short reads such as Hi-C, 10x or Illumina data\n");
         exit(1);
    }
    ...
    for(i=1;i<argc;i++)
    {
       ...
       else if(!strcmp(argv[i],"-help"))
       {
         ...
         printf("Usage: breakpoint %s -nodes 30 -reads input_step.fasta <Input_Reference> <Output_Stones-file>\n",argv[0]);
         printf("       nodes    (30)    - Number of CPUs requested\n");
         exit(1);
       }
...
/*      Plot coverage frequency chr by chr */
    ...
    if((strcmp(datatype,"ccs") == 0))
      data_flag = 1;
    if((strcmp(datatype,"ont") == 0))
      data_flag = 2;
    if((strcmp(datatype,"ngs") == 0))
      data_flag = 3;
...
//    sprintf(file_ctspec,"%s/%s.spec",tempa,argv[args+1]);
	...
	if(ed != NULL)
	  ...
	  {
            memset(syscmd,'\0',2000);
            sprintf(syscmd,"%s/pigz -dc %s > reads_ont.fq ",bindir,file_genome);
            RunSystemCommand(syscmd);
	  ...
	  {
            memset(syscmd,'\0',2000);
            sprintf(syscmd,"%s/step_shred -rlength 8000 %s reads_ont.shred ",bindir,file_genome);
            RunSystemCommand(syscmd);
	  }
        }
        else

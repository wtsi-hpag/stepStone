...
int main(int argc, char **argv)
{
    ...
    if(argc < 2)
    {
         ...
         printf("With aligned, and name sorted sam,bam or cram files:\n");
         printf("Usage: %s -nodes 30 -data ccs/ont/ngs-HiC/ngs-10X/ngs-SSR -bam align.bam <output_breakpoints.vcf>\n",argv[0]);
         printf("=== Note: the sam/bam/cram file has to be Name sorted! ===\n");
         printf("=== If not read name sorted, please do the following:  ===\n");
         printf("=== samtools sort -n your.bam new.bam ===\n\n");

         printf("With fasta/fastq read files:\n");
         printf("Usage: %s -nodes 30 -data ccs/ont/ngs -reads input_reads.fasta/q <reference> <output_breakpoints.vcf>\n",argv[0]);
    ...
    for(i=1;i<argc;i++)
    {
       ...
       else if(!strcmp(argv[i],"-help"))
       {
         ...
         printf("Usage: breakpoint %s -nodes 60 -reads input_step.fasta <Input_Reference> <Output_Stones-file>\n",argv[0]);
         printf("Usage: plot %s -nodes 60 -bam /myspace/desk/test-sorted.bam -sample cancer\n",argv[0]);
         printf("Usage: align %s -nodes 60 -data ccs/ont -reads input_step.fasta <Input_Reference> <Output_sorted_bam>\n",argv[0]);
         printf("Usage: align %s -nodes 60 -data ngs-HiC -fq1 read_1.fq.gz -fg2 read_2.fq.gz <Input_Reference> <Output_sorted_bam>\n",argv[0]);
         printf("Usage: align %s -nodes 60 -data ngs-10X -fq1 read_1.fq.gz -fg2 read_2.fq.gz <Input_Reference> <Output_sorted_bam>\n",argv[0]);
         printf("Usage: align %s -nodes 60 -data ngs-SSR -fq1 read_1.fq.gz -fg2 read_2.fq.gz <Input_Reference> <Output_sorted_bam>\n",argv[0]);
         printf("       nodes    (30)    - Number of CPUs requested\n");
         exit(1);
       }
    ...
    if((strcmp(datatype,"ccs") == 0))
      ...
      data_flag = 5;
    else if((strcmp(datatype,"ont-CLR") == 0))
    {
      data_flag = 2;
      clr_flag = 1;
    }
